...
float Adafruit_BME280::readData(float &_Tem, float &_Pre, float &_lvl)
{
	int64_t var1, var2, var3, var4;

	_Tem = readTemperature(); // must be done first to get t_fine
	...
	float atmospheric = _Pre / 100.0F;
	_lvl = 44330.0 * (1.0 - pow(atmospheric / _seaLevelPre, 0.1903));
	return _lvl;
}
